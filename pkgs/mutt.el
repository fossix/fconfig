;; mutt.el --- Use Emacs as an external editor for the Mutt mailer
...
;;;
;;; $Log: mutt.el,v $
;;; Revision 1.1  2004/06/24 05:08:22  grog
;;; Initial revision
;;;
;;; Revision 1.1  2004/06/24 05:08:15  grog
;;; Initial revision
;;;
...
(defun mutt-goto-signature ()
  ...
  (interactive)
  (goto-char (point-max))
  (and (search-backward-regexp (concat "^" mutt-signature-pattern
				       "[ \t\f]*$"))
       (previous-line 1)))
...
(defun headers-goto-field (field)
  (let ((case-fold-search t))
    (goto-char (point-min))
    (when (search-forward-regexp (concat "^\\($\\|" field ": \\)"))
      (if (looking-at "^$")
	  (progn
	    (insert-string field ": \n")
	    (forward-char -1))
...
(defun headers-attach-file (file)
